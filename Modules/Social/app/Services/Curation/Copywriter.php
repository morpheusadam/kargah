...
class Copywriter
{
    ...
    private function prompt(Story $story, ?string $articleText, array $briefs): string
    {
        ...
        return <<<PROMPT
            ...
            🔴 عدد کمینه‌ی هر شبکه یک الزام است، نه یک پیشنهاد. اگر برای شبکه‌ای ۱۸ هشتگ
            خواسته شده، باید ۱۸ تا بدهی. واژگان بالا برای رسیدن به ۱۸ کافی نیست و
            نباید هم باشد — راه رسیدن این است:

            **هر اسم خاصِ لاتینی که در خودِ متن خبر آمده را هم به شکل هشتگ بیاور.**
            نام شرکت، نام محصول، نام سرویس، نام فناوری: #Cloudflare، #OpenAI،
            #IronMountain، #CloudStorage. فقط لاتین، بدون فاصله، بدون underscore، و
            فقط اگر آن کلمه واقعاً در متن هست. اسم از خودت نساز — چیزی که در متن
            نباشد حذف می‌شود.

            برای شبکه‌ای که کمینه‌اش بالاست، ترتیب کار این است: اول دسته، بعد هر
            برچسب مرتبط از واژگان، بعد هر اسم خاص لاتین متن تا به عدد برسی. اگر با
            همه‌ی این‌ها هم به کمینه نرسیدی، همان تعداد را بده — ولی اول واقعاً
            بگرد.
